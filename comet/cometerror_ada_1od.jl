...
for method in method_string
    local comp_time = @elapsed begin
        local vel_num, pos_num, times = launch_from_comet_1ord(initial_vel, initial_pos, tspan, beta, method,adaptive=true)
    end
...
# Create the plot
p = plot(title="1nd Order ODE Solvers", xlabel="Time [days]", ylabel="Error [AU]")

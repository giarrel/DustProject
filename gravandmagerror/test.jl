...
sol = solve(prob, RK4(), adaptive=false, dt=stepsize)
...
v_num = [norm(u[4:6]) for u in sol.u]

r_numeric = [norm(u[1:3]) for u in sol.u]

times = sol.t

rel_err= [abs(v_num[i]-v_anal(r_numeric[i]))/v_anal(r_numeric[i]) for i in 1:length(times)]

# Plot the results
figure = lines(x_vel_numeric/AU,x_vel_numeric/AU)
save("gravmagplots/orbit.png", figure)
